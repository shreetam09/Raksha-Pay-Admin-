...
function useAccount() {
  ...
  if (!context) throw new Error('Account context is missing');
  return context;
}
            ...
            <div className="mb-6" key={group.label}>
              ...
              <div className="space-y-1">
                {group.items.map((item) => {
                  const isAccounts = item.label === 'Accounts' || item.href === '/accounts';
                  const targetHref = isAccounts ? `/accounts/${activeAccountId || 'rp-092'}` : item.href;
                  const active = item.href === '/'
                    ? location === '/'
                    : isAccounts
                      ? location.startsWith('/accounts')
                      : location.startsWith(item.href.split('/').slice(0, 2).join('/'));
                  const Icon = item.icon;
                  return (
                    <Link key={`${group.label}-${item.label}`} href={targetHref} onClick={onClose} data-testid={`link-nav-${item.label.toLowerCase().replaceAll(' ', '-')}`} className={`group flex h-10 items-center gap-3 rounded-lg px-3 text-[12px] font-semibold transition-colors ${active ? 'bg-primary text-primary-foreground' : 'text-sidebar-foreground/60 hover:bg-sidebar-accent hover:text-sidebar-foreground'}`}>
...
function AccountDetail() {
  ...
  const targetId = params?.id || activeAccountId || accounts[0]?.id;
  const account = accounts.find((item) => item.id === targetId) || accounts[0];
  const { notify } = useFeedback();
  const [held, setHeld] = useState(false);

  useEffect(() => {
    if (params?.id && params.id !== activeAccountId) {
      setActiveAccountId(params.id);
    }
  }, [params?.id, activeAccountId, setActiveAccountId]);
...
function AppRouter({ theme, onToggle }) {
  return (
    <Shell theme={theme} onToggle={onToggle}>
      <Switch>
        <Route path="/" component={Dashboard} />
        <Route path="/accounts" component={AccountDetail} />
...
function App() {
  ...
  const [activeAccountId, setActiveAccountId] = useState(() => localStorage.getItem('rakshapay-active-account') || accounts[0]?.id || 'rp-092');
  ...
  const updateActiveAccountId = (id) => {
    ...
    try {
      localStorage.setItem('rakshapay-active-account', id);
    } catch {}
  };

  useEffect(() => {
    document.documentElement.classList.toggle('dark', theme === 'dark');
    localStorage.setItem('rakshapay-theme', theme);
  }, [theme]);
  ...
  return (
    <FeedbackContext.Provider value={{ notify: setToast }}>
      <AccountContext.Provider value={{ activeAccountId, setActiveAccountId: updateActiveAccountId }}>
